...
if len(sys.argv) > 1:
   ...
   print("execute script algorithms/styLshapes/find_styLshp_wo_pixch_btwn_frames.py file1 " + filename1 + " file2 " + filename2 + " directory " + directory )
...
for im1shapeid in im1shapes:
   ...
   if len( cur_im1shp_pixels ) < Lshape_size:
      continue
...
for im2shapeid in im2shapes:
   ...
   if len( cur_im2shp_pixels ) < Lshape_size:
      continue
...
max_progress_num = len( im1shapes_wo_pixch )
cur_progress_num = len( im1shapes_wo_pixch )
prev_progress_num = len( im1shapes_wo_pixch )
remaining_chars = ""
      ...
      while end is False:
         ...
         for im2shapeid in im2shapes_wo_pixch:
            ...
            if run_count >= 2 and len( cur_matched_shapes ) >= 1:
               ...
               print("ERROR at " + os.path.basename(__file__) + " line " + sys._getframe().f_lineno )
               sys.exit(1)
            
      # displaying closest match images/

      if len( cur_matched_shapes ) >= 1 and display_or_not is True:
