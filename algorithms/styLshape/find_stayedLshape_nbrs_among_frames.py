   ...
   if prev_styLshapes_w_neighbors is None:
      ...
      for prev_shapeid in prev_styLshapes_w_neighbors:
         ...
         for cur_shapeid in styLshapes_w_neighbors:
            ...
            # check if found_pixels are more than 60% of either prev_cur_im2shapes or cur_im2shapes
            if len( found_pixels ) >= len( prev_cur_im2pixels ) * 0.65 or len( found_pixels ) >= len( cur_im2pixels ) * 0.65:
               # match is found. now check if previous shapeid is present in shapes_among_frames
               found_in_shp_amng_frm = False
               for lindex, each_shapes_among_frames in enumerate( shapes_among_frames ):
                  # each_shapes_among_frames -> ['prev10.11_cur11.12', '2072.1', '4497.1']
...
styLshapes_w_neighbors_dfile = styLshapes_w_neighbors_Ddir + "among.data"
