...
data_dir = pathlib.Path(spixcShp_w_same_Lshapes_ddir)
...
for ordered_btwn_frames_file in ordered_btwn_frames_files:
   ...
   with open (spixcShp_w_same_Lshapes_ddir + ordered_btwn_frames_file + ".data", 'rb') as fp:
      # [['2', '26000', 0.9470517448856799], ... ]
      # [ [ image1 shapeid, image2 shapeid, match count ], ... ]
      spixcShp_w_same_Lshapes = pickle.load(fp)
   fp.close()


   # looping current 2 frames matches
   for btwn_frames_match in spixcShp_w_same_Lshapes:
...
for each_shapes_among_frames in shapes_among_frames:
   ...
   for filename in each_shapes_among_frames:
      ...
      shape_image = spixcShp_w_same_Lshapes_dir + filename + "/" + each_shapes_among_frames[filename] + ".png"
