...
def move_pixels( shapes_pixels, move_direction, directly_UD, step, move_total, notfnd_shapes, notfnd_shapes_by_pindex, notfnd_shapes_clrs, notfnd_shapes_nbrs, min_colors ):
      ...
      for move_UD in range( 0, move_total, step ):
         ...
         for shape_pixels in shapes_pixels:
            ...
            for xy in shape_pixels[2]:
               ...
               for temp_shapeid in notfnd_shapes_by_pindex[ pindex ]:
                  # check if current shape's color is the same as not found image's pixel
                  if min_colors is True:
                     if shape_pixels[1] != notfnd_shapes_clrs[ temp_shapeid ]:
                        continue
                  else:
                     appear_diff = pixel_functions.compute_appearance_difference( shape_pixels[1] , notfnd_shapes_clrs[ temp_shapeid ] )
                     if appear_diff is True:
                        # different appearance
                        continue
               
                  if temp_shapeid not in pixel_found_from_notfnd_shapes.keys():
                     pixel_found_from_notfnd_shapes[ temp_shapeid ] = 1
                  else:
                     pixel_found_from_notfnd_shapes[ temp_shapeid ] += 1
...
im_height = None
im_size = None
frth_smallest_pixc = third_smallest_pixc = None
...
with open(added_missed3_dfile, 'wb') as fp:
