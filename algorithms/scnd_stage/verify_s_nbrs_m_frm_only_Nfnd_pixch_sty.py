...
for shapes_match in all_shapes_matches:
   ...
   if check_highest_attached_nbrs[1] is False:
      for im1nbr in im1shapes_nbrs:
         ...
         im1im2nbr_match = same_shapes_functions.match_shape_while_moving_it( im1shp_coord_xy, im2shp_coord_xy )
         im2im1nbr_match = same_shapes_functions.match_shape_while_moving_it( im2shp_coord_xy, im1shp_coord_xy )
         if im2im1nbr_match is True and im1im2nbr_match is True:
...
# verifying by neighbor of pixch staying shapes
# check if shape has pixch staying shape as its neighbor
# check which shape inside shapes_match is neighbor to it. Let's say image1 shape1 is neighbor to pixch staying shape.
# find out image1 shape1's matched shape in image2. see if this matched shape in image2 is neighbor to matched image2 pixch staying shape.
for shapes_match in all_shapes_matches:
   # shapes_match -> [['3295', '2086'], ['2875', '2872', '2885']]

   if shapes_match in verified_shapes:
      continue

   im1shapes_nbrs = set()
   for im1shape in shapes_match[0]:
      im1shapes_nbrs |= set( im1shapes_neighbors[im1shape] )


   pixch_sty_s_nbrs = set()
   # check if im1shapes_nbrs contain pixch staying shape.
   for im1shape_nbr in im1shapes_nbrs:
      found_pixch_sty_s_nbrs = { shapes for shapes in pixch_stayed_shapes if shapes[0] == im1shape_nbr }
   
      pixch_sty_s_nbrs |= found_pixch_sty_s_nbrs
   
   
   im2shapes_nbrs = set()
   for im2shape in shapes_match[1]:
      im2shapes_nbrs |= set( im2shapes_neighbors[im2shape] )

   im2pixch_sty_s_nbrs = set()
   # check if im2shapes_nbrs contain pixch_staying shape
   for im2shape_nbr in im2shapes_nbrs:
      found_im2pixch_sty_s_nbrs = { shapes for shapes in pixch_stayed_shapes if shapes[1] == im2shape_nbr }
      
      im2pixch_sty_s_nbrs |= found_im2pixch_sty_s_nbrs
   
   # check if found pixch staying shape neighbors match
   pixch_sty_s_nbr_match_threshold = 0.35
   
   found_pixch_sty_s_nbr_matches = pixch_sty_s_nbrs.intersection( im2pixch_sty_s_nbrs )
   
   if len( found_pixch_sty_s_nbr_matches ) > 0 and len( found_pixch_sty_s_nbr_matches ) / len( pixch_sty_s_nbrs ) >= 0.35:
      # matched
      verified_shapes.append( shapes_match )
