...
for data_file in ordered_files:
   ...
   scnd_stage_alg_shp2_dfile = only_nfnd_pixch_sty2_Dir + data_file + ".data"
   ...
   mid_ch_dfile = mid_ch_dir + "data/" + data_file + "verified.data"
   with open (mid_ch_dfile, 'rb') as fp:
      # {('56582', '57397'), ('34163', '35293'), ... }
      mid_ch_shapes = pickle.load(fp)
   fp.close()
   
   cur_all_sty_shapes |= mid_ch_shapes
   ...
   if prev_all_sty_shapes is None:
      ...
      prev_scnd_stg_algo_shapes = scnd_stage_alg_shp2
      ...
      for each_prev_shapes in prev_all_sty_shapes:
         ...
         if len( found_shapes ) == 0:
            ...
            found_shapes = [ True for temp_shapes in scnd_stage_alg_shp2 if each_prev_shapes[1] in temp_shapes[0] ]
            
            if len( found_shapes ) >= 1:
               verified_shapes[ prev_filename + "_" + data_file ].append( [ each_prev_shapes[0], each_prev_shapes[1], "", "" ]  )  
      ...
      for each_prev_scnd_s_a_shapes in prev_scnd_stg_algo_shapes:
         # each_prev_scnd_s_a_shapes -> [['3120', '11916'], ['4712', '11515']]
         
         for prev_im2scnd_shape in each_prev_scnd_s_a_shapes[1]:
            found_shapes = { True for temp_shapes in cur_all_sty_shapes if prev_im2scnd_shape[1] == temp_shapes[0] }
            
            if len( found_shapes ) >= 1:
               verified_shapes[ prev_filename + "_" + data_file ].append( [ "", prev_im2scnd_shape, "", "" ]  )     
            
            else:
               found_shapes = { True for temp_shapes in scnd_stage_alg_shp2 if prev_im2scnd_shape == temp_shapes[0] }
...
for each_files in all_shapes:
   cur_shapes_pairs = set()
   for each_shapes in all_shapes[each_files]:
      if each_shapes[0] == "" or each_shapes[1] == "":
         # matched shapes pair contain empty shape?
         print("attention required in cross_check_all_low_level_algo_acrs_all_files.py. empty shape is found " + 
               " in matched shapes pair. is this allowed?")
         sys.exit()
