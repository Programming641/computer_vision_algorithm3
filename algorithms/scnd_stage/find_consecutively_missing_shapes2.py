...
for each_file in all_matches_so_far:
   ...
   if prev_file_shapes is None:
      ...
      prev_im1shapes_by_pindex = im1shapeids_by_pindex
      prev_im2shapes_by_pindex = im2shapeids_by_pindex
               ...
               if len( prev_common_nbrs_found ) >= 1:
                  ...
                  for prev_common_nbr_found in prev_common_nbrs_found:
                     ...
                     for prev_im2nfnd_shape_pix in prev_im2shapes[each_shapes[0]]:
                        ...
                        for prev_temp_im1shapeid in prev_im1shapes_by_pindex[pindex]:
                           if prev_temp_im1shapeid in prev_im1shapes_in_relpos.keys():
                              prev_im1shapes_in_relpos[ prev_temp_im1shapeid ] += 1
                           else:
                              prev_im1shapes_in_relpos[ prev_temp_im1shapeid ] = 1
                     ...
                     for prev_im1shapeid in prev_im1shapes_in_relpos:
                        if len( prev_im1shapes[ prev_im1shapeid ] ) < frth_smallest_pixc:
                           continue
                        
                        if min_colors is True:
                           if prev_im2shapes_clrs[ each_shapes[0] ] != prev_im1shapes_clrs[ prev_im1shapeid ]:
                              continue
                        else:
                           appear_diff = pixel_functions.compute_appearance_difference(prev_im2shapes_clrs[ each_shapes[0] ] , prev_im1shapes_clrs[ prev_im1shapeid ] )
                     ...
                     for prev_found_matched_im1nbr in prev_found_matched_im1nbrs:
                        ...
                        for possible_prev_im1match in prev_im1shapes_nbrs[ prev_found_matched_im1nbr ]:
                           ...
                           for im1not_found_shape_pix in prev_im1shapes[possible_prev_im1match]:
                              ...
                              for prev_temp_im2shapeid in prev_im2shapes_by_pindex[pindex]:
                                 if prev_temp_im2shapeid in prev_im2shapes_in_relpos.keys():
                                    prev_im2shapes_in_relpos[ prev_temp_im2shapeid ] += 1
                                 else:
                                    prev_im2shapes_in_relpos[ prev_temp_im2shapeid ] = 1
                           ...
                           for prev_im2shapeid in prev_im2shapes_in_relpos:
                              if len( prev_im2shapes[ prev_im2shapeid ] ) < frth_smallest_pixc:
                                 continue
            
                              if min_colors is True:
                                 if prev_im1shapes_clrs[ possible_prev_im1match ] != prev_im2shapes_clrs[ prev_im2shapeid ]:
                                    continue
                              else:
                                 appear_diff = pixel_functions.compute_appearance_difference(prev_im1shapes_clrs[ possible_prev_im1match ], prev_im2shapes_clrs[ prev_im2shapeid ] )
                                 if appear_diff is True:
                                    # different appearance
                                    continue                             
               ...
               if len( cur_common_nbrs_found ) >= 1:
                  ...
                  for cur_common_nbr_found in cur_common_nbrs_found:
                     ...
                     for prev_im2nfnd_shape_pix in prev_im2shapes[prev_each_shapes[1]]:
                        ...
                        for temp_im2shapeid in im2shapeids_by_pindex[pindex]:
                           if temp_im2shapeid in cur_im2shapes_in_relpos.keys():
                              cur_im2shapes_in_relpos[ temp_im2shapeid ] += 1
                           else:
                              cur_im2shapes_in_relpos[ temp_im2shapeid ] = 1
                     ...
                     for cur_im2shapeid in cur_im2shapes_in_relpos:
                        if len( im2shapes[ cur_im2shapeid ] ) < frth_smallest_pixc:
                           continue
                        
                        if min_colors is True:
                           if im1shapes_colors[ prev_each_shapes[1] ] != im2shapes_colors[ cur_im2shapeid ]:
                              continue
                        else:
                           appear_diff = pixel_functions.compute_appearance_difference(im1shapes_colors[ prev_each_shapes[1] ], im2shapes_colors[ cur_im2shapeid ] )
                           if appear_diff is True:
                              # different appearance
                              continue                        
                     ...
                     for cur_found_matched_im2nbr in cur_found_matched_im2nbrs:
                        ...
                        for possible_im2match in im2shapes_neighbors[ cur_found_matched_im2nbr ]:
                           ...
                           for possible_im2match_pix in im2shapes[possible_im2match]:
                              ...
                              for temp_im1shapeid in im1shapeids_by_pindex[pindex]:
                                 if temp_im1shapeid in im1shapes_in_relpos.keys():
                                    im1shapes_in_relpos[ temp_im1shapeid ] += 1
                                 else:
                                    im1shapes_in_relpos[ temp_im1shapeid ] = 1
                           ...
                           for im1shapeid in im1shapes_in_relpos:
                              ...
                              if min_colors is True:
                                 if im2shapes_colors[ possible_im2match ] != im1shapes_colors[ im1shapeid ]:
                                    continue
                              else:
                                 appear_diff = pixel_functions.compute_appearance_difference(im2shapes_colors[ possible_im2match ], im1shapes_colors[ im1shapeid ] )
                                 if appear_diff is True:
                                    # different appearance
                                    continue
...
if os.path.exists(missed_shapes2_dfile):
   with open (missed_shapes2_dfile, 'rb') as fp:
