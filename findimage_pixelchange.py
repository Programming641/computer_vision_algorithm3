...
im1file = "hanger001_color_group"
im2file = "hanger002_color_group"
...
for y in range(im1height):
  ...
  for x in range(im1width):
    ...
    if im1pxls[pixel_index] != im2pxls[pixel_index] and ( im1pxls[pixel_index] != (255, 255, 255) and im2pxls[pixel_index] != (255, 255, 255) ) :
       print("current pixel changed! which is located at x " + str(x) )
       print(im2pxls[pixel_index])
       
       newdata.append( im2pxls[pixel_index] )
...
pxlch_im.save ("images/" + directory + "diff12result2.png")
