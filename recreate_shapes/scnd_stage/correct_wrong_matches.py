...
correct_matches_dir = top_shapes_dir + directory + scnd_stg_all_files + "/correct_matches/"
correct_matches_ddir = correct_matches_dir + "data/"
correct_matches_dfile = correct_matches_ddir + algorithm_num + ".data"
with open (correct_matches_dfile, 'rb') as fp:
   # {'10.11': [('37042', '43422'), ('40641', '43422'), ... ], ... }
   # { prev filename_cur filename: [ ( image1 shapeid, image2 shapeid), ... ], ... }
   wrong_matches = pickle.load(fp)
fp.close()


across_all_files_dir = correct_matches_dir + algorithm_num

for filename in wrong_matches:
   print( filename )
   ...
   for each_file_shapes in wrong_matches[filename]:
