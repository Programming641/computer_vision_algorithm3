...
nfnd_shapes_ddir = top_shapes_dir + directory + scnd_stg_all_files + "/possible_matches/data/" + file_num + ".data"
with open (nfnd_shapes_ddir, 'rb') as fp:
   # {'10.11': [('37042', '43422'), ('40641', '43422'), ... ], ... }
   # { prev filename_cur filename: [ ( image1 shapeid, image2 shapeid), ... ], ... }
   possible_shapes = pickle.load(fp)
fp.close()


possible_shapes_files_dir = top_shapes_dir + directory + scnd_stg_all_files + "/possible_matches/" + file_num
for filename in possible_shapes:
   print( filename )

   im1file = filename.split(".")[0]
   im2file = filename.split(".")[1]
   
   across_all_files_imdir = possible_shapes_files_dir + "." + im1file + "." + im2file + "/"
   ...
   for each_file_shapes in possible_shapes[filename]:
